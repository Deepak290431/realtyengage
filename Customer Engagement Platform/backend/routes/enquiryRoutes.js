...
router.post('/',
  ...
  async (req, res) => {
    try {
      const { projectId, enquiryType, details, priority, preferredContactMethod, preferredContactTime } = req.body;
...
router.put('/:id',
  ...
  [
    param('id').isMongoId(),
    body('status').optional().isIn(['new', 'in_progress', 'follow_up', 'converted', 'closed', 'rejected']),
    body('priority').optional().isIn(['low', 'medium', 'high']),
    body('assignedTo').optional().isMongoId(),
    body('followUpDate').optional().isISO8601(),
    body('isOk').optional().isBoolean()
  ],
  handleValidationErrors,
  async (req, res) => {
    try {
