...
const projectSchema = new mongoose.Schema({
  ...
  status: {
    type: String,
    enum: ['upcoming', 'in_progress', 'completed', 'ongoing'],
    required: true,
    default: 'upcoming'
  },
  viewers: [String], // Store unique viewer identifiers (emails or user IDs)
  type: {
    type: String,
  ...
  amenities: [{
    ...
    icon: {
      type: String // name of the icon (e.g., 'Waves', 'Dumbbell')
    }
  }],
  specifications: [{
    label: String,
...
projectSchema.pre('save', function (next) {
  ...
  if (this.availability.totalUnits === undefined || this.availability.totalUnits === null) {
    this.availability.totalUnits = 1;
  }
